...
import club.banyuan.mbm.service.SupplierService;
  ...
  private void process(MbmRequest mbmRequest) throws IOException {
    switch (mbmRequest.getPath()) {
      ...
      case "/server/provider/list": {
        ...
        String payload = mbmRequest.getPayload();
//        System.out.println(mbmRequest.getPayload().trim());
        if (payload == null) {
          supplierList = supplierService.getSupplierList();
        } else {
//          payload = payload.trim().replace("\"flag\":\"search\"", "");
...
////        id=0&name=张三&desc=123&phone=13012345678&contactPerson=李四&button=提交
//        int endIndex = mbmRequest.getPayload().lastIndexOf("&");
//        String payload = mbmRequest.getPayload().substring(0, endIndex);
//        mbmRequest.setPayload(payload);
//        System.out.println(mbmRequest);
        Map<String, String> formData = mbmRequest.getFormData();
        String data = JSONObject.toJSONString(formData);
        Supplier supplier = JSONObject.parseObject(data, Supplier.class);
        if (supplier.getId() == 0) {
