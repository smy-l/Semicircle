...
SELECT * FROM fruits WHERE f_price BETWEEN 2 AND 10.20;

-- 查询价格在2.00元到10.20元之外的水果名称和价格
SELECT * FROM fruits WHERE f_price NOT BETWEEN 2 AND 10.20;
...
SELECT * FROM employee WHERE (YEAR(CURDATE()) - YEAR(hireDate)) > 20;
...
SELECT o_num, SUM(item_price * quantity) as 总订单价格 
FROM orderitems 
GROUP BY o_num 
