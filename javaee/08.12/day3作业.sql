...
alter table tb_emp add constraint emp_dept_id_fk foreign key (deptId) references tb_dept(id);
...
create table tb_emp1(
  ...
  deptId INT(11),
  salary FLOAT,
	constraint emp1_dept_id_fk foreign key (deptId) references tb_dept(id)
);
...
alter table tb_emp drop foreign key emp_dept_id_fk;
alter table tb_emp add constraint emp_dept1_id_fk foreign key (deptid) references tb_dept1(id);
...
select c_name, grade from score sg inner join student st on st.id = sg.stu_id and st.name = '李四';

-- 查询所有学生的信息和考试信息
select * from student st inner join score sc on st.id = sc.stu_id;

-- 计算每个学生的总成绩
select stu_id, sum(grade) 总成绩 from score GROUP BY stu_id;

-- 计算每个考试科目的平均成绩
select c_name, avg(grade) 每科平均成绩 from score GROUP BY c_name;
